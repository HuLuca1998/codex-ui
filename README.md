...
### GitHub Actions 云端自动发布

推送到 `main` 后，GitHub Actions 会在公开仓库免费的 ARM64 `macos-15` Runner 上
自动测试、构建并发布。版本、Tag、Release 和 ZIP 文件名统一使用当前提交的 7 位 hash。
...
同一 hash 已存在 Release 时，工作流会安全结束，不会重复上传。
